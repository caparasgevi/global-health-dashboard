...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50 transition-all duration-300 font-poppins">
        ...
        >
          <div className="relative w-10 h-10 md:w-12 md:h-10 flex items-center justify-center group-hover:scale-105 group-active:scale-95 transition-transform duration-200 overflow-hidden">
            {!logoError ? (
              ...
              <span
                className={`text-[10px] font-bold ${isDark ? "text-white/70" : "text-black/70"}`}
              >
                HR
              </span>
            )}
        ...
        <nav className="hidden xl:flex items-center bg-gray-100/50 dark:bg-gray-800/40 p-1.5 rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
          {navItems.map((item) => (
            ...
            >
              {item}
              <span
                className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 bg-brand-red transition-all duration-300 rounded-full ${activeItem === item ? "w-4 opacity-100" : "w-0 opacity-0"}`}
              />
            </button>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
          ...
          {user && !isGuest ? (
            <div className="hidden md:flex items-center gap-4 pr-3 mr-1 border-r border-gray-200 dark:border-gray-700">
              {/* ADDED: Profile button from old code - navigates to /profile on click, shows "View Profile" on hover */}
              <button
              ...
              >
                ...
                <span className="text-[11px] font-black text-gray-800 dark:text-white group-hover:text-brand-red uppercase tracking-wider leading-none truncate max-w-[100px]">
                  {(
              ...
              <button
                onClick={onLogout}
                className="text-[10px] font-bold text-gray-400 hover:text-brand-red uppercase tracking-widest transition-colors"
              >
                Logout
              </button>
            </div>
          ...
          <button
            ...
            className="flex xl:hidden flex-col justify-center items-center w-10 h-10 rounded-xl bg-brand-red text-white shadow-lg shadow-brand-red/20 active:scale-95 transition-transform"
          >
            <div
              className={`w-5 h-0.5 bg-white transition-all duration-300 mb-1 ${isMenuOpen ? "rotate-45 translate-y-1.5" : ""}`}
            />
            <div
              className={`w-5 h-0.5 bg-white transition-all duration-300 mb-1 ${isMenuOpen ? "opacity-0" : ""}`}
            />
            <div
      ...
      {/* Mobile Menu */}
      <div
        className={`xl:hidden fixed inset-x-0 top-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-2xl transition-all duration-300 origin-top ${isMenuOpen ? "scale-y-100 opacity-100 visible" : "scale-y-0 opacity-0 invisible"}`}
      >
        <div className="flex flex-col p-4 gap-2">
          {navItems.map((item) => (
            <button
              key={item}
              onClick={() => scrollToSection(item)}
              className={`w-full text-left px-5 py-4 rounded-xl text-sm font-black uppercase tracking-widest transition-all ${activeItem === item ? "bg-brand-red/10 text-brand-red border-l-4 border-brand-red" : "text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
            >
              {item}
            </button>
          ))}
          ...
          <div className="mt-2 pt-4 border-t border-gray-100 dark:border-gray-800">
            {user && !isGuest ? (
              <div className="flex items-center justify-between px-5 py-2">
                {/* ADDED: Profile navigation on mobile username tap as well */}
                <button
                  onClick={() => {
                    navigate("/profile");
                    setIsMenuOpen(false);
                  }}
                  className="text-xs font-black text-gray-800 dark:text-white uppercase hover:text-brand-red transition-colors"
                >
                  {user.user_metadata?.name ||
                    user?.email?.split("@")[0] ||
                    "User"}
                ...
                <button
                  onClick={onLogout}
                  className="text-sm font-black text-brand-red uppercase"
                >
                  Logout
                </button>
              </div>
              ...
              <button
                onClick={handleSignIn}
                className="w-full text-left px-5 py-4 text-sm font-black text-brand-red uppercase"
              >
                Sign In
              </button>
            )}
