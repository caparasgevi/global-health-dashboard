...
const SEVERITY_CONFIG = {
  critical: { label: "Critical — Active Outbreak", dotColor: "#dc2626", badgeBg: "#fef2f2", badgeText: "#dc2626", animated: true },
  high: { label: "High — Monitor", dotColor: "#ea580c", badgeBg: "#fff7ed", badgeText: "#ea580c", animated: true },
  moderate: { label: "Moderate — Advisory", dotColor: "#ca8a04", badgeBg: "#fefce8", badgeText: "#ca8a04", animated: false },
  watch: { label: "Watch — Informational", dotColor: "#2563eb", badgeBg: "#eff6ff", badgeText: "#2563eb", animated: false },
};
...
async function fetchAlertsForCountry(countryName: string, region: string, signal?: AbortSignal): Promise<CountryData> {
  ...
  const indicatorData = (await Promise.all(indicatorPromises)).filter((i): i is NonNullable<typeof i> => i !== null);
  ...
  indicatorData.sort((a, b) => b.value - a.value).slice(0, 3).forEach(ind => {
    alerts.push({
      id: `ind-${ind.code}`,
  ...
  const result = {
    ...
    stats: {
      activeCases: activeAlertsCount > 0 ? `${activeAlertsCount} active` : "Monitoring",
...
async function detectUserLocation() {
  try {
    ...
    return { country: data.country_name || "United States", region: data.region || "Nationwide" };
  } catch { return { country: "United States", region: "Nationwide" }; }
}

const SeverityBadge: React.FC<{ severity: HealthAlert["severity"] }> = ({ severity }) => {
  const cfg = SEVERITY_CONFIG[severity];
  return (
    <span className="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md mb-2" style={{ background: cfg.badgeBg, color: cfg.badgeText }}>
      <span className="w-1.5 h-1.5 rounded-full" style={{ background: cfg.dotColor, animation: cfg.animated ? "hrPulse 1.5s ease-in-out infinite" : "none" }} />
      {cfg.label}
    </span>
  );
};
...
const SmallAlertCard: React.FC<{ alert: HealthAlert }> = ({ alert }) => (
  ...
  >
    ...
    <div className="p-5 flex-1 flex flex-col">
      <h3 className="text-sm font-black tracking-tight mb-2 line-clamp-2 text-slate-900 dark:text-white leading-snug group-hover:text-brand-red transition-colors">{alert.title}</h3>
      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 mb-4 leading-relaxed">{alert.description}</p>
      <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100 dark:border-slate-800/50">
        <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">{alert.date}</span>
        <span className="text-[10px] font-black text-brand-red uppercase tracking-widest">{alert.source}</span>
      </div>
    </div>
  </div>
);
...
const CountryHealthAlerts: React.FC<CountryHealthAlertsProps> = ({ userCountry, userRegion }) => {
  ...
  useEffect(() => {
    mountedRef.current = true; loadAlerts();
  ...
  return (
    <section id="country-health-alerts" className="w-full py-8 px-0 font-poppins">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-brand-red">Local Disease Surveillance Network</span>
            <span className="flex-1 h-px bg-brand-red/20" />
          </div>
          <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white font-montserrat tracking-tight">Tracking Threats in {displayCountry || "Your Region"}</h2>
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Real-time outbreak monitoring based on WHO data</p>
        </div>

        {loading ? <AlertsSkeleton /> : error || !countryData ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
              {[
                ...
                { label: "Health Indicators", value: countryData.stats.totalTracked, color: "text-slate-900 dark:text-white" },
                { label: "Data Coverage", value: countryData.stats.vaccinationRate, color: "text-emerald-500 dark:text-emerald-400" },
              ].map(stat => (
                <div key={stat.label} className="rounded-2xl p-5 flex flex-col items-center justify-center text-center border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-900/50 min-h-[100px]">
                  <span className={`block text-3xl font-black tracking-tighter ${stat.color}`}>{stat.value}</span>
                  <span className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1.5">{stat.label}</span>
                </div>
              ))}
            </div>
          </div>
