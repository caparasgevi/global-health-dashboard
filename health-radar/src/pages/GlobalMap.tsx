...
import { Map, MapMarker, MapControls, MarkerContent, MarkerTooltip, type MapRef } from "../components/ui/Map";
          ...
          >
            <MarkerContent>
              <div className="group relative flex flex-col items-center">
                ...
                <div className="pointer-events-none absolute bottom-full mb-2 hidden group-hover:block">
                  <div className="rounded-lg bg-slate-900 px-2 py-1 text-[10px] text-white shadow-xl">
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/90 p-4 backdrop-blur-md">
          ...
          <div className="relative">
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute mt-2 w-full rounded-xl bg-slate-900 border border-white/10 shadow-2xl">
                {suggestions.map(s => (
                  <button 
                    key={s.id} 
                    onClick={() => flyToCountry(s)}
                    className="w-full px-4 py-2 text-left text-xs text-slate-300 hover:bg-white/10 first:rounded-t-xl last:rounded-b-xl"
                  >
                    {s.country}
                  </button>
                ))}
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/90 p-4 backdrop-blur-md text-white">
          ...
          <div className="space-y-3">
            {topOutbreaks.map((o, i) => (
              <div key={o.id} onClick={() => flyToCountry(o)} className="flex items-center justify-between cursor-pointer hover:opacity-70">
