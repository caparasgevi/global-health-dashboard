...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchRegionalData = async () => {
      setIsLoading(true);
      try {
        const rawData = await healthService.getGlobalBaseline({
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 text-center md:text-left">
          <div className="flex-1">
            ...
            <p className="text-lg md:text-xl text-slate-500 dark:text-slate-400 font-medium italic opacity-80">
          ...
          <div className="theme-card rounded-[2rem] p-6 md:p-8 flex flex-col bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
            <div className="mb-6">
              <div className="flex items-start justify-between gap-2 mb-1">
                <h2 className="text-sm font-black uppercase text-slate-400 dark:text-slate-500 font-montserrat tracking-[0.2em]">
                  Risk Assessment
                </h2>
                <button
                ...
                >
                  <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/50 p-3">
                    ...
                    <div className="grid grid-cols-[1fr_auto] gap-x-3 mb-1 px-1">
                      ...
                      <span className="text-[8px] font-black uppercase tracking-widest text-slate-300 dark:text-slate-600 text-right">
                        Weight
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      {THREAT_INDICATORS.map((ind) => (
                        <div
                          key={ind.code}
                          className="grid grid-cols-[1fr_auto] gap-x-3 items-center px-1 py-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/40 transition-colors"
                        >
                          <div>
                            <p className="text-[10px] font-bold text-slate-700 dark:text-slate-300 leading-tight">
                              {ind.label}
                            </p>
                            <p className="text-[8px] font-medium text-slate-400 dark:text-slate-500 font-mono leading-tight">
                              {ind.code}
                            </p>
                          </div>
                          <span className="text-[10px] font-black text-brand-orange tabular-nums">
                            {ind.weight}
                          </span>
                        </div>
                      ))}
                    ...
                    <p className="text-[8px] leading-relaxed text-slate-400 dark:text-slate-500 mt-2 pt-2 border-t border-slate-200 dark:border-slate-700/50">
                      Each indicator is normalised to 0–100 against a
                      disease-specific ceiling, then combined as a weighted
                      average. Only indicators with regional data contribute.
                      Scores &gt; 60 →{" "}
                      <span className="text-brand-red font-black">HIGH</span> ·
                      &gt; 25 →{" "}
                      <span className="text-brand-orange font-black">
                        MODERATE
                      </span>{" "}
                      · else{" "}
                      <span className="font-black text-slate-500">LOW</span>.
                    </p>
                ...
                <>
                      ...
                      >
                        <div className="flex justify-between items-end mb-2.5">
                          <div className="flex flex-col">
                            <span className="text-xs font-black invisible uppercase tracking-tighter mb-0.5">
                              Region
                            </span>
                          ...
                          <div className="text-right">
                            <span className="text-[10px] font-black invisible uppercase block mb-0.5">
                              Index
                            </span>
                            <span
                              className={`text-xs font-black tracking-widest uppercase ${item.threatLevel > 70 ? "text-brand-red" : "text-brand-orange"}`}
                            >
                              {item.status} ·{" "}
                              <Counter value={item.threatLevel} />%
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="flex items-start gap-3 opacity-60 grayscale hover:grayscale-0 transition-all">
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src =
                          "https://www.svgrepo.com/show/306988/world-health-organization.svg";
                      }}
                    />
                    <p className="text-[9px] leading-relaxed text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">
                      Official Surveillance Data provided by WHO APIs.
                      <br />
