      ...
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          ...
          <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-6">
            {['Sign In', 'Sign Up'].map((label, i) => (
              <button key={label} type="button"
                onClick={() => { setIsLogin(i === 0); setWasSubmitted(false); setShowCountryDropdown(false); setAuthError(null); }}
        ...
        <m.div layout transition={{ duration: 0.25, ease: "easeInOut" }}>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <AnimatePresence>
              {authError && (
                <m.div initial={{ opacity: 0, y: -10, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }} exit={{ opacity: 0, y: -10, height: 0 }} className="overflow-hidden">
                  <div className="flex items-center gap-3 p-3 mb-2 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 text-xs font-medium">
                    ...
                    <button type="button" onClick={() => setAuthError(null)} className="p-1 hover:bg-red-100 dark:hover:bg-red-500/20 rounded-md transition-colors"><X size={14} /></button>
                  </div>
                </m.div>
              )}
              {resetSent && (
                <m.div initial={{ opacity: 0, y: -10, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }} exit={{ opacity: 0, y: -10, height: 0 }} className="overflow-hidden">
                  <div className="flex items-center gap-3 p-3 mb-2 rounded-xl bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 text-green-700 dark:text-green-400 text-xs font-medium">
                    ...
                    <span className="flex-1">Check your email for the reset link!</span>
                  </div>
                </m.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {!isLogin && !isForgotPassword && (
                <m.div key="signup-fields" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-4 pb-4">
                  {/* Full Name */}
                  <div>
                    <label className={labelClass}>Full Name</label>
                    <input type="text" required placeholder="Full Name" className={getInputClass(formData.name.length > 0)} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                    {wasSubmitted && !formData.name && <span className="text-[10px] text-red-500 font-semibold ml-0.5 mt-0.5 block">* Required</span>}
                  </div>

                  {/* Country searchable dropdown */}
                  <div className="relative" ref={countryRef}>
                    <label className={labelClass}>Country / Region <span className="text-brand-red">*</span></label>
                    <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border-2 transition-all cursor-pointer ${wasSubmitted && !formData.country ? 'border-red-400' : showCountryDropdown ? 'border-brand-red/40 ring-2 ring-brand-red/8' : 'border-slate-200 dark:border-slate-700'}`} onClick={() => setShowCountryDropdown(v => !v)}>
                      ...
                      <span className={`flex-1 text-sm font-medium ${formData.country ? 'text-slate-800 dark:text-white' : 'text-slate-400'}`}>{formData.country || 'Select your country'}</span>
                      <svg className={`w-3.5 h-3.5 text-slate-400 transition-transform ${showCountryDropdown ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </div>

                    <AnimatePresence>
                      {showCountryDropdown && (
                        <m.div initial={{ opacity: 0, y: -4, scaleY: 0.95 }} animate={{ opacity: 1, y: 0, scaleY: 1 }} exit={{ opacity: 0, y: -4, scaleY: 0.95 }} transition={{ duration: 0.12 }} style={{ transformOrigin: 'top' }} className="absolute z-[9999] mt-1.5 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden">
                          <div className="p-2 border-b border-slate-100 dark:border-slate-700">
                            <input autoFocus type="text" placeholder="Search country..." value={formData.countrySearch} onChange={(e) => setFormData({ ...formData, countrySearch: e.target.value })} className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-700 rounded-lg outline-none border border-slate-200 dark:border-slate-600 placeholder:text-slate-400 dark:text-white font-medium focus:border-brand-red/40" onClick={(e) => e.stopPropagation()} />
                          </div>
                          <ul className="max-h-48 overflow-y-auto">
                            {filteredCountries.length === 0 ? (
                              <li className="px-4 py-3 text-sm text-slate-400 text-center">No countries found</li>
                            ) : filteredCountries.map((c) => (
                              <li key={c.alpha2} onClick={() => { setFormData({ ...formData, country: c.country, countrySearch: '' }); setShowCountryDropdown(false); }} className={`flex items-center gap-3 px-4 py-2.5 text-sm cursor-pointer transition-colors ${formData.country === c.country ? 'bg-red-50 dark:bg-red-950/30 text-brand-red font-semibold' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                                <span>{c.country}</span>
                                {formData.country === c.country && <svg className="w-3.5 h-3.5 ml-auto text-brand-red" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>}
                              </li>
                            ))}
                          </ul>
                        </m.div>
                    ...
                    {wasSubmitted && !formData.country && <span className="text-[10px] text-red-500 font-semibold ml-0.5 mt-0.5 block">* Required</span>}
                  </div>
                  ...
                  <div className="flex items-start gap-3 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 rounded-xl px-4 py-3">
                    <input id="alerts" type="checkbox" checked={formData.alertsEnabled} onChange={(e) => setFormData({ ...formData, alertsEnabled: e.target.checked })} className="mt-0.5 accent-brand-red w-3.5 h-3.5 shrink-0 cursor-pointer" />
                    <label htmlFor="alerts" className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed cursor-pointer">
                      <span className="font-bold text-slate-700 dark:text-slate-300">Enable outbreak alerts</span><br />
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {isLogin && (
                      <button 
                        type="button" 
                        onClick={() => { setIsForgotPassword(true); setAuthError(null); setWasSubmitted(false); }} 
            ...
            <div className="pt-2">
              <m.button layout type="submit" whileTap={{ scale: 0.98 }} disabled={resetSent}
                ...
                {isForgotPassword ? (resetSent ? 'Link Sent' : 'Send Reset Link') : (isLogin ? 'Sign In' : 'Create Account')}
              </m.button>

              {isForgotPassword && (
                <button 
                  type="button" 
                  onClick={() => { setIsForgotPassword(false); setAuthError(null); setResetSent(false); }}
